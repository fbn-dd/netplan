...
function dhtmlXTreeObject(htmlObject,width,height,rootId){
 ...
 this._gserverlIdStorage=new Array();
 this.gserverlNodeStorage=new Array();
 this._gserverlIdStorageSize=0;
...
function dhtmlXTreeItemObject(itemId,itemText,parentObject,treeObject,actionHandler,mode){
 ...
 this.id=treeObject._gserverlIdStorageAdd(itemId,this);
 ...
 dhtmlXTreeObject.prototype._gserverlIdStorageAdd=function(itemId,itemObject){
 if(this._gserverlIdStorageFind(itemId,1,1)){d=new Date();itemId=d.valueOf()+"_"+itemId;return this._gserverlIdStorageAdd(itemId,itemObject);}
 this._gserverlIdStorage[this._gserverlIdStorageSize]=itemId;
 this.gserverlNodeStorage[this._gserverlIdStorageSize]=itemObject;
 this._gserverlIdStorageSize++;
 return itemId;
};
 
 dhtmlXTreeObject.prototype._gserverlIdStorageSub=function(itemId){
 for(var i=0;i<this._gserverlIdStorageSize;i++)
 if(this._gserverlIdStorage[i]==itemId)
{
 this._gserverlIdStorage[i]=this._gserverlIdStorage[this._gserverlIdStorageSize-1];
 this.gserverlNodeStorage[i]=this.gserverlNodeStorage[this._gserverlIdStorageSize-1];
 this._gserverlIdStorageSize--;
 this._gserverlIdStorage[this._gserverlIdStorageSize]=0;
 this.gserverlNodeStorage[this._gserverlIdStorageSize]=0;
}
};
 
 
 dhtmlXTreeObject.prototype._gserverlIdStorageFind=function(itemId,skipXMLSearch,skipParsing){
 
 for(var i=0;i<this._gserverlIdStorageSize;i++)
 if(this._gserverlIdStorage[i]==itemId)
{
 return this.gserverlNodeStorage[i];
}
 ...
 if(parentObject.childsCount>=2)
{
 ...
 var parentObject=this._gserverlIdStorageFind(parentId);
 ...
 if((!dhtmlObject.waitUpdateXML)||(dhtmlObject.waitUpdateXML.toString().search(","+cId+",")!=-1))
{
 ...
 var temp=dhtmlObject._gserverlIdStorageFind(parentId);
 temp.XMLload=1;
 ...
 if((node.childNodes[i].nodeType==1)&&(node.childNodes[i].tagName == "userdata"))
{
 ...
 var parsedNodeTop=dhtmlObject._gserverlIdStorageFind(dhtmlObject.parsingOn);
 ...
 if(!itemObject){
 var tempx=dhtmlObject._gserverlIdStorageFind(dhtmlObject.lastLoadedXMLId);
...
{
 var temp=this._gserverlIdStorageFind(itemId);
 if(!temp)return 0;
 this._xopenAll(temp);
};
 ...
 dhtmlXTreeObject.prototype.closeAllItems=function(itemId)
{
 ...
 var sNode=this._gserverlIdStorageFind(itemId);
 if(!sNode)return;
...
{
 if(this.lastSelected)
 if(this._gserverlIdStorageFind(this.lastSelected.parentObject.id))
 ...
 dhtmlXTreeObject.prototype.changeItemId=function(itemId,newItemId)
{
 ...
 if(this._gserverlIdStorage[i]==itemId)
{
 this._gserverlIdStorage[i]=newItemId;
}
};
...
dhtmlXTreeObject.prototype._checkParenNodes=function(itemId,htmlObject,shtmlObject){
 ...
 itemObject.id=this._gserverlIdStorageAdd(itemObject.id,itemObject);
 itemObject.treeNod=this;
 if(oldTree)oldTree._gserverlIdStorageSub(itemObject.id);
 for(var i=0;i<itemObject.childsCount;i++)
 ...
 for(var i=0;i<Count;i++)
{
 ...
 if(!skip){
 this._gserverlIdStorageRecSub(htmlObject);
}
...
dhtmlXTreeObject.prototype._deleteItem=function(itemId,selectParent,skip){
 ...
 if(!skip){
 this._gserverlIdStorageRecSub(sNode);
 ...
 dhtmlXTreeObject.prototype._gserverlIdStorageRecSub=function(itemObject){
 for(var i=0;i<itemObject.childsCount;i++)
{
 this._gserverlIdStorageRecSub(itemObject.childNodes[i]);
 this._gserverlIdStorageSub(itemObject.childNodes[i].id);
};
 this._gserverlIdStorageSub(itemObject.id);
};
 ...
 var sNode=this._gserverlIdStorageFind(parentItemId);
 ...
 var z=this._gserverlIdStorageFind(itemId);
