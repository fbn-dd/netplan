...
SERVICE=apcupsd
SERVICEDESC=$5
# wenn apcupsd abstürzt, dann läuft der Prozess noch als Zombie und blockiert einen restart, da hilft nur killen und das pidfile ist schon leer
RESTARTCMD="/usr/bin/ssh -o BatchMode=yes -o StrictHostKeyChecking=no root@$SERVER /usr/bin/killall $SERVICE \&\& /etc/init.d/$SERVICE start"
